...
public class AlertOthersPanicGoal extends Goal {
    ...
    public boolean canUse() {
        if (this.mob.getLastHurtByMob() == null || !this.mob.getLastHurtByMob().isAlive()) {
            ...
            if (this.mob.isOnFire()) {
                BlockPos blockpos = this.lookForWater(this.mob.level(), this.mob);
                if (blockpos != null) {
    ...
    protected BlockPos lookForWater(BlockGetter level, Entity entity) {
        ...
        return !level.getBlockState(blockpos).getCollisionShape(level, blockpos).isEmpty() ? null : BlockPos.findClosestMatch(entity.blockPosition(), 5, 1, (fluidState) -> level.getFluidState(fluidState).is(FluidTags.WATER)).orElse(null);
    }
}
